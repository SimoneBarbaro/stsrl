...
logging.basicConfig(filename=os.path.join(dir_path, "logs", f"test-communicator{time.time()}.log"), encoding='utf-8', level=logging.DEBUG)
print("Logging to: ", os.path.join(dir_path, "test-communicator.log"))
            ...
            if self.last_error is None:
                ...
                if self.in_game:
                    ...
                    try:
                        ...
                        logger.error("Failed to load game context because: %s", e.__str__())
    ...
    if sts_action.action_type == sts.SeachActionType.CARD:
        ...
        for i in range(len(game_state.monsters)):
            if game_state.monsters[i].is_gone:
                ...
                sts_action.target_idx -= 1
                if sts_action.target_idx < 0:
                    break
    ...
    if sts_action.action_type == sts.SeachActionType.POTION:
        ...
        for i in range(len(game_state.monsters)):
            if game_state.monsters[i].is_gone:
                ...
                if sts_action.target_idx < 0:
                    break
        return PotionAction(potion_index=sts_action.source_idx, target_index=target_idx)
...
class TestAgent(SimpleAgent):
    ...
    def get_next_action_in_game(self, state):
        try:
            ...
            logger.error("Error in get_next_action_in_game: %s", e.__str__())
            return super().get_next_action_in_game(state)
