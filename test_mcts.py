...
from torchrl.objectives import DQNLoss, LossModule, distance_loss
...
logging.basicConfig(filename=os.path.join(dir_path, "logs", f"test-mcts.log"),
                    level=logging.INFO)


class FilterValidActionsNoiseModule(DirichletNoiseModule):
    def __init__(self,
                 alpha: float = 0.3,
                 epsilon: float = 0.3,
                 only_root: bool = True,
                 ...
                 action_mask_key: NestedKey = "legal_actions_mask"):
        super().__init__(alpha, epsilon, only_root, prior_action_value_key, is_root_key)
        self.action_mask_key = action_mask_key

    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        tensordict = super().forward(tensordict)
        tensordict[self.prior_action_value_key][tensordict[self.action_mask_key] == 0] = 0
        return tensordict
...
class Experiment:
    ...
    def make_env(
        ...
        transforms = (
            StepCounter(),
            DTypeCastTransform(
                in_keys=["legal_actions_mask"], out_keys=["legal_actions_mask"],
                in_keys_inv=[],
                dtype_in=torch.float32, dtype_out=torch.bool),
            CatTensors(["game", "battle"], "observation")

        )

        if self.make_actor_explore == self.make_mcts_actor:
            transforms = transforms + (TruncateTrajectory(self.truncate_key),)
        base_env = GymWrapper(gym.make('sts'), device="cpu", categorical_action_encoding=True)
    ...
    def make_mcts_actor(
        ...
        mcts_policy = MctsPolicy(
            expansion_strategy=AlphaZeroExpansionStrategy(
                ...
                value_module=actor,
                noise_module=FilterValidActionsNoiseModule(action_mask_key="legal_actions_mask")
            ),
    ...
    def make_model(
        ...
        actor = QValueActor(
            module=qvalue_net, spec=env.action_spec, action_mask_key="legal_actions_mask"
        )
    ...
    def make_logger(self) -> Logger:
        exp_name = os.path.join("runs", f"{self.exp_prefix}{datetime.now().isoformat().replace(':', '_')}")
