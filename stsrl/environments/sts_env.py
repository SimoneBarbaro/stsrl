        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "game": gym.spaces.Box(np.zeros_like(StsEncodings.nniInstance.getObservationMaximums()),
                                       np.ones_like(StsEncodings.nniInstance.getObservationMaximums())),
    ...
    def _get_obs(self):
        if self.gc.screen_state == sts.ScreenState.BATTLE:
            return {
                ...
                "battle": StsEncodings.encode_battle(self.gc, self.bc) / StsEncodings.nniInstance.getBattleObservationMaximums(),
    ...
    def step(self, action):
        ...
        if self.is_battle():
            ...
            if self.is_battle():
                ...
                self.turn = -1
        # Let's try giving rewards along the way to guide the agent to choices that favor higher final score?
